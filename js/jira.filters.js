...
function JiraFilter(param, _loader){
	var server = (param.server)?((typeof param.server == 'string')?((_loader.servers.get(param.server)!=-1)?_loader.servers.get(param.server):_loader.servers.default):param.server):_loader.servers.default,
		timer = null,
		data = null,
		self = this,
		loader = _loader||window.loader,
		notifications = [];
	self.columns = {
		type: true,
		key: true,
	...
	self.show = (typeof param.show != 'undefined')?param.show:'table';
	...
	this.__defineGetter__("server", function(){ return server['id']; });
	this.__defineSetter__("server", function(sid){ console.log(window,loader); server = loader.servers.get(sid); });
	
	self.__defineGetter__('keys', function(){ var sKeys = server.getItem(".keys." + self.id); return (sKeys)?sKeys.split(","):[];});
	self.__defineSetter__('keys', function(keys){ server.setItem(".keys." + self.id, keys.toString()); });
	if(self.type=='jql'){
		self.jql  = param.jql;
			...
			$.each(self.issues, function(i, val){
					...
					if(self.keys.length && $.inArray(val[1], self.keys)<0){
							...
							notification.onclick = function(event){
								loader.addTab(server.url +"/browse/"+val[1]); 
							...
							notification.onclose = function(event){
								notifications = $.map(notifications,function(notif){
